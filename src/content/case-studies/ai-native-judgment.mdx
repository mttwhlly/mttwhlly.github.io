...
description: >
  Two AI-integration decisions I made inside the same organization, using the same evaluation
  discipline: I adopted MCP as versioned, org-wide developer infrastructure on one system, and
  built an MCP tool layer plus a vector-search path for a production AI search product on another
  — then removed both once I confirmed neither had the shape the technology was built to solve. A
  reusable framework for matching integration pattern to system shape, not to what's currently
  trending.
publishedDate: '2026'
order: 2
draft: false
