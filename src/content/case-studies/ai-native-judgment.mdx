...
description: >
  ...
  discipline: I adopted MCP as versioned, org-wide infrastructure serving a dozen-plus vendor pods
  on one system — then built and removed a five-tool MCP layer plus a vector-search path on
  another once I confirmed it had no shape the technology was built to solve. A reusable framework
  for matching integration pattern to system shape, not to what's currently trending.
publishedDate: '2026'
order: 2
draft: true
